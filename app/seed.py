...
def ensure_user(db, company_id: str, full_name: str, email: str, role_key: str):
...
def ensure_employee(db, company_id: str, employee_code: str, full_name: str, user_id: str | None = None):
...
def main():
    ...
    md_user = ensure_user(db, company_id, "Meera Doggzi", "md@doggzi.example", "MD")
    hr_user = ensure_user(db, company_id, "Harsha HR", "hr@doggzi.example", "HR")
    finance_user = ensure_user(db, company_id, "Finley Finance", "finance@doggzi.example", "FINANCE")
    emp_one = ensure_user(db, company_id, "Esha Employee", "employee1@doggzi.example", "EMPLOYEE")
    emp_two = ensure_user(db, company_id, "Ravi Employee", "employee2@doggzi.example", "EMPLOYEE")

    ensure_employee(db, company_id, "EMP-1001", "Esha Employee", str(emp_one["_id"]))
    ensure_employee(db, company_id, "EMP-1002", "Ravi Employee", str(emp_two["_id"]))
